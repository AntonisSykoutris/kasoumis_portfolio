...
const HeroAnimatedText = ({ charDelay }) => {
    ...
    >
      {text.split(' ').map((word, wordIndex) => (
        <motion.div
          ...
          className='inline-block text-[44px] leading-10 xs:text-[3.2rem] xs:leading-[3.2rem] sm:text-6xl lg:text-[6rem] font-arcane 1xl:text-[10rem]  '
        >
          {word}
          {wordIndex < text.split(' ').length - 1 && <>&nbsp;</>}
        </motion.div>
      ))}
      <SparklesText text='༼ つ ◕_◕ ༽つ' />
    </motion.div>
  );
};
